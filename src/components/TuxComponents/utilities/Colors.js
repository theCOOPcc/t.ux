// Tux Colors
export const tux_blue = "#78d7da";
export const tux_yellow = "#f0eb7e";
export const tux_red = "#E6211E";
export const tux_grey = "#323239";
export const tux_white = "#FAFCFE";
export const tux_black = "#17171e";
export const tux_orange = '#F37806';

export const success = 'rgba(120, 192, 119, 1)';
export const true_white = "#fff";

export const common_font_color =
'rgba(235, 237, 238, 1)';

// Link styles
export const link_font_color = 
'rgba(5, 0, 255, 0.75)';
export const link_default = '#518e90';
export const link_focus = '#a9cbcd';
export const link_hover = 'rgba(169, 203, 205, 1)';
export const link_disabled = '#888888';


export const link_text = "#538284";
export const grey_text = "#666666";
export const intro_text = "#366062";
export const present_text = "#1B9BA0";
export const future_text = "#999999";
export const text_black = "#000";

// Flag Feature
export const hoverOption = "#FEF0E2";
export const selectOption = "#FEDEC1";

// ********* Button Element Colors *********

export const disable_btn = "#ccc";
export const pressed_btn = '#1C3C3B';
// Primary Button Colors
// Default and Focus colors are the same
export const primary_hover = '#C2ECEE';
export const primary_text = '#274d4f';
// Secondary Button Colors
export const secondary_default = '#3c8582';
export const secondary_hover = '#6da4a1';


// TODO: remove enable_btn, enable_hover, default_focus, and default_depressed once all instances of default button are removed and replaced with Secondary button
export const enable_btn = "#3c8582";
export const enable_hover = "#236c69";
export const default_focus = "#1C3C3B";
export const default_depressed = "#6DA4A1";
export const google_hover = "#E5E5E5";
export const disable_g_btn = "#D3D3D3";
export const g_focus_box = "#b4c8e9";
export const small_nav_default = "#3c8582";
export const small_nav_hover = "#6DA4A1";
export const small_nav_pressed = "#1C3C3B";
export const small_nav_disabled = "#DDDDDD";
export const small_nav_focus = "rgba(250, 252, 254, 1)";

// Input Colors
export const background_input = "#eee";
export const tux_primart = "#366062";
export const success_grey = "rgba(136, 136, 136, 0.5)";
export const dark_grey = "#444444";
export const medium_grey = "#666666";
export const white = "white";

//Lozenge Colors
export const complete = "#78C077";
export const in_progress = "#f37806";
export const not_started = "#333333";
//Background Colors
export const backgroundBlur = "rgba(170, 170, 170, 0.6)";
export const pageBackground = "#E5E5E5";
export const backgroundGradient =
  "linear-gradient(210.65deg, #FFEE99 17.3%, rgba(122, 218, 222, 0.32) 87.56%), linear-gradient(19.08deg, #EA4A46 -33.26%, rgba(234, 74, 70, 0) 67.74%);";
export const grey = "#DDDDDD";
export const backButton = "#222222";